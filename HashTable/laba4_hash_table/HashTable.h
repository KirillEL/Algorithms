...
template<class Key, class Data>
int HashTable<Key, Data>::convert(Key key) { // Преобразование методом свертки
...
template<class Key, class Data>
int HashTable<Key, Data>::hash(int key) { // хеширование
...
bool HashTable<Key, Data>::showMode() {
    if (isChain) {
        cout << "Цепочки коллизий\n";
    }
    else cout << "Открытая адресация\n";
    return isChain;
}
